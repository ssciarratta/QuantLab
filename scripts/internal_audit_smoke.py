...
def check_f47_chat_context() -> None:
    ...
    assert PHASES_SUMMARY == "F19–F81 INTERNAL"
...
def check_about_version_matches() -> None:
    ...
    assert __version__ == "0.73.0"
    assert __version__.startswith("0.73")
    assert PHASES_SUMMARY == "F19–F81 INTERNAL"
...
def check_version_starts_with_073() -> None:
    """F81: tip version on 0.73.x line."""
    from quantlab import __version__
    from quantlab.workbench.about import PHASES_SUMMARY, build_about_payload
    ...
    assert PHASES_SUMMARY == "F19–F81 INTERNAL"
    about = build_about_payload()
    assert about["version"].startswith("0.73")
    assert about["version"] == __version__
    assert about["live_blocked"] is True
...
def main() -> int:
    checks: list[tuple[str, Callable[[], None]]] = [
        ...
        ("version starts with 0.73", check_version_starts_with_073),
