...
def check_about_version_matches() -> None:
    ...
    assert __version__ == "0.83.0"
    assert __version__.startswith("0.83")
    assert PHASES_SUMMARY == "F19–F91 INTERNAL"
...
def check_version_starts_with_083() -> None:
    ...
    assert PHASES_SUMMARY == "F19–F91 INTERNAL"
    about = build_about_payload()
    assert about["version"].startswith("0.83")
    assert about["version"] == __version__
    assert about["live_blocked"] is True
...
def main() -> int:
    checks: list[tuple[str, Callable[[], None]]] = [
        ...
        ("version starts with 0.83", check_version_starts_with_083),
