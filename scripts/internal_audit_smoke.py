...
def check_about_version_matches() -> None:
    ...
    assert __version__ == "0.79.0"
    assert __version__.startswith("0.79")
    assert PHASES_SUMMARY == "F19–F87 INTERNAL"
...
def check_version_starts_with_079() -> None:
    """F87: tip version on 0.79.x line."""
    from quantlab import __version__
    from quantlab.workbench.about import PHASES_SUMMARY, build_about_payload
    ...
    assert PHASES_SUMMARY == "F19–F87 INTERNAL"
    about = build_about_payload()
    assert about["version"].startswith("0.79")
    assert about["version"] == __version__
    assert about["live_blocked"] is True
...
def main() -> int:
    checks: list[tuple[str, Callable[[], None]]] = [
        ...
        ("version starts with 0.79", check_version_starts_with_079),
