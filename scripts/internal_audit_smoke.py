...
def check_version_starts_with_070() -> None:
    """F78: tip version on 0.70.x line (Milestone Freeze v0.70)."""
    from quantlab import __version__
    from quantlab.workbench.about import PHASES_SUMMARY, build_about_payload
    ...
    assert PHASES_SUMMARY == "F19–F78 INTERNAL"
    about = build_about_payload()
    assert about["version"].startswith("0.70")
    assert about["version"] == __version__
    assert about["live_blocked"] is True
...
def main() -> int:
    checks: list[tuple[str, Callable[[], None]]] = [
        ...
        ("version starts with 0.70", check_version_starts_with_070),
