...
def check_f55_openapi() -> None:
    ...
    assert ("POST", "/api/live/demo/submit") in paths
    assert ("GET", "/api/live/demo/fills") in paths
...
def check_f101_binance_demo_routing() -> None:
    """F101: demo submit post-unlock (local sim); sin unlock bloqueado."""
    import os
    ...
    from quantlab.workbench.about import PHASES_SUMMARY
    from quantlab.workbench.api import ApiError, WorkbenchState, handle_post_live_demo_submit
    from quantlab.workbench.session import WorkbenchSession
    ...
    assert (root / "docs" / "FASE_101_BINANCE_DEMO_ROUTING.md").is_file()
    assert not (root / "docs" / "audit" / "FASE_101_APPROVED.md").exists()

    smoke_root = _smoke_tmp("quantlab-smoke-f101-demo")
    if smoke_root.exists():
        import shutil

        shutil.rmtree(smoke_root, ignore_errors=True)
    smoke_root.mkdir(parents=True, exist_ok=True)
    session = WorkbenchSession.create_or_load(smoke_root, "f101")
    state = WorkbenchState(session=session, session_parent=smoke_root)
    try:
    ...
    os.environ["QUANTLAB_LIVE_USER"] = "smoke_op"
    os.environ["QUANTLAB_LIVE_PASSWORD"] = "smoke_secret"
    try:
        unlock_live_session(username="smoke_op", password="smoke_secret")
        router = LiveOrderRouter()
        assert router.status()["transport"] == "local_demo_sim"
        out = handle_post_live_demo_submit(
            state, {"symbol": "ETHUSDT", "side": "SELL", "quantity": "0.01"}
        )
        assert out["ok"] is True
        ...
        reset_live_unlock_for_tests()
        os.environ.pop("QUANTLAB_LIVE_USER", None)
        os.environ.pop("QUANTLAB_LIVE_PASSWORD", None)


def check_f102_binance_testnet() -> None:
    """F102: testnet opt-in gated; default local; rejects production host."""
    from quantlab import __version__
    ...
    try:
        BinanceTestnetClient(
            api_key="k", api_secret="s", base_url="https://api.binance.com"
        )
        raise AssertionError("expected production reject")
    except ValidationError:
        pass
    root = Path(__file__).resolve().parents[1]
    assert (root / "src" / "quantlab" / "brokers" / "binance" / "testnet_client.py").is_file()
    assert (root / "docs" / "FASE_102_BINANCE_TESTNET.md").is_file()
    assert not (root / "docs" / "audit" / "FASE_102_APPROVED.md").exists()


def check_f103_live_arc_freeze() -> None:
    """F103: freeze docs arco LIVE Guided Path F99–F102."""
    from quantlab import __version__
    from quantlab.execution.live_gate import LIVE_BLOCKED
    from quantlab.workbench.about import PHASES_SUMMARY
...
def check_f104_guided_lab_a3() -> None:
    """F104: Guided Lab A3 paper connect UI."""
    from quantlab import __version__
    from quantlab.execution.live_gate import LIVE_BLOCKED
    from quantlab.workbench.about import PHASES_SUMMARY
    ...
    assert "QLApi.connect" in js
    assert (root / "docs" / "FASE_104_GUIDED_LAB_A3.md").is_file()
    assert not (root / "docs" / "audit" / "FASE_104_APPROVED.md").exists()


def check_f105_a3_md_guided() -> None:
    """F105: A3 MD status API + Guided Lab md_source env."""
    from quantlab import __version__
    ...
    assert "gl-a3-md" in js and "a3MdStatus" in js
    assert (root / "docs" / "FASE_105_A3_MD_GUIDED.md").is_file()
...
def check_f107_a3_paper_submit_guided() -> None:
    """F107: Guided Lab A3 paper submit post-connect."""
    from quantlab import __version__
    from quantlab.execution.live_gate import LIVE_BLOCKED
    from quantlab.workbench.about import PHASES_SUMMARY
    ...
    assert "gl-a3-paper" in js and "QLApi.paperSubmit" in js
    assert (root / "docs" / "FASE_107_GUIDED_LAB_A3_PAPER_SUBMIT.md").is_file()
    assert not (root / "docs" / "audit" / "FASE_107_APPROVED.md").exists()


def check_f110_guided_arc_freeze() -> None:
    """F110: Milestone freeze arco Guided F99-F109."""
    from quantlab import __version__
    from quantlab.execution.live_gate import LIVE_BLOCKED
    from quantlab.workbench.about import PHASES_SUMMARY
...
def check_f109_live_demo_complete() -> None:
    """F109: LIVE demo cancel + LIMIT + mirror."""
    from quantlab import __version__
    from quantlab.execution.live_gate import LIVE_BLOCKED
    from quantlab.workbench.about import PHASES_SUMMARY
    ...
    assert "gl-demo-price" in js and "gl-demo-mirror" in js
    assert "liveDemoCancel" in js or "QLApi.liveDemoCancel" in js
    assert (root / "docs" / "FASE_109_LIVE_DEMO_COMPLETE.md").is_file()
    catalog = (root / "src" / "quantlab" / "workbench" / "api_catalog.py").read_text(
        encoding="utf-8"
    )
    assert "/api/live/demo/cancel" in catalog
    assert "/api/live/demo/open-orders" in catalog


def check_f108_guided_lab_i18n() -> None:
    """F108: Guided Lab i18n keys."""
    import json
    ...
    en = json.loads(
        (root / "src" / "quantlab" / "workbench" / "static" / "i18n" / "en.json").read_text(
            encoding="utf-8"
        )
    )
    gl_es = {k for k in es if k.startswith("guided_lab.")}
    gl_en = {k for k in en if k.startswith("guided_lab.")}
    assert gl_es == gl_en and len(gl_es) >= 40
    js = (
        root / "src" / "quantlab" / "workbench" / "static" / "js" / "panes" / "guided_lab.js"
    ).read_text(encoding="utf-8")
...
def main() -> int:
    checks: list[tuple[str, Callable[[], None]]] = [
        ...
        ("F101 binance demo routing post-unlock", check_f101_binance_demo_routing),
        ("F102 binance spot testnet opt-in", check_f102_binance_testnet),
        ...
        ("F107 A3 paper submit guided lab", check_f107_a3_paper_submit_guided),
