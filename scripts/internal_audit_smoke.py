...
def check_about_version_matches() -> None:
    ...
    assert __version__ == "0.80.0"
    assert __version__.startswith("0.80")
    assert PHASES_SUMMARY == "F19–F88 INTERNAL"
...
def check_version_starts_with_080() -> None:
    """F88: tip version on 0.80.x line."""
    from quantlab import __version__
    from quantlab.workbench.about import PHASES_SUMMARY, build_about_payload
    ...
    assert PHASES_SUMMARY == "F19–F88 INTERNAL"
    about = build_about_payload()
    assert about["version"].startswith("0.80")
    assert about["version"] == __version__
    assert about["live_blocked"] is True
...
def main() -> int:
    checks: list[tuple[str, Callable[[], None]]] = [
        ...
        ("version starts with 0.80", check_version_starts_with_080),
