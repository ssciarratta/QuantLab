...
(function (global) {
  ...
  function safetyBadge(opts) {
    ...
    var modeLabel = mode === "paper" ? "PAPER" : mode.toUpperCase();
    var blocked = opts.liveBlocked !== false;
    var parts = [modeLabel, "DATOS BINANCE"];
    if (blocked) parts.push("PRODUCCIÓN BLOQUEADA");
    else parts.push("LIVE DESBLOQUEADO");
    if (opts.venue) parts.push(String(opts.venue).toUpperCase());
  ...
  /** @param {{ steps: Array<{label:string,paneId?:string}>, onStep?: function(string) }} opts */
